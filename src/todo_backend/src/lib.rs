...
type TodoStore = HashMap<String, Todo>;
type TodoOrder = Vec<String>; // To Maintain Order of TODOS
...
fn create_todo(name: String, description: String) -> Result<String, String> {
...
#[query(name = "get_todo")]
fn get_todo(todo_id: String) -> Result<Todo, String> {
...
fn get_todos(page: usize, page_size: Option<usize>) -> Vec<Todo> {
    ...
    let todos = TODOORDER.with(|todo_order| {
        ...
        let end_index = usize::min(start_index + page_size, order.len());
